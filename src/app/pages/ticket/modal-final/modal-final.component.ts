...
export class ModalFinalComponent implements OnInit {
  ...
  getFinalPrice() {
    this.managerForm.valueChanges.subscribe((el) => {
      ...
      ).discount; // act like finalprice
      this.discountDisplay = +fp;
      this.cdr.detectChanges();
      console.log("🍻 this.discountDisplay ------------", this.discountDisplay);
    });
  ...
  affectationFinalPrice() {
    ...
    let finalpv = this.caculateDiscount(
      this.rowData.finalPrice,
      this.valueSlider ? this.valueSlider.toString() : "0"
    ).finalPrice;
    console.log("🥜 discount", discount);
    console.log("🥜 finalpv", finalpv);
